...
class ObservacoesLimitacoesSection(BaseSection):
    def render(self, story, styles, dados_parseados, contexto_extra):
        ...
        story.append(Paragraph(body, styles["texto"]))
        aprovacao = get_section_prose(contexto_extra, "observacoes_limitacoes", "aprovacao", "")
        if aprovacao:
            story.append(Spacer(1, 12))
            story.append(Paragraph(f"<b>{aprovacao}</b>", styles["texto"]))
            story.append(Paragraph("Aprovação / Coordenação CEM", styles["texto"]))
        story.append(Spacer(1, 10))
